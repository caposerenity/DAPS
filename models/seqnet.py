...
class SeqNet(nn.Module):
    ...
    def inference(self, images, targets=None,is_source=True, query_img_as_gallery=False):
        ...
        if targets is not None and not query_img_as_gallery:
            ...
            detections, _ = self.roi_heads(
                features, proposals, images.image_sizes, targets,is_source, query_img_as_gallery
            )
    ...
    def forward(self, images_s, targets_s=None, images_t=None, targets_t=None,is_source=True, query_img_as_gallery=False):
        if not self.training:
            return self.inference(images_s, targets_s, is_source, query_img_as_gallery)

        # print(images[0].shape, images[1].shape, images[2].shape, images[3].shape, images[4].shape)
        images_s, targets_s = self.transform(images_s, targets_s)
        images_t, targets_t = self.transform(images_t, targets_t)
        # print(images.tensors.shape)
        # print(images[0].shape)
        features = self.backbone(images_s.tensors)
        proposals, proposal_losses = self.rpn(images_s, features, targets_s)
        _, detector_losses = self.roi_heads(features, proposals, images_s.image_sizes, targets_s, is_source=True)
        ...
        proposals, proposal_losses_t = self.rpn(images_t, features, targets_t)
        _, detector_losses_t = self.roi_heads(features, proposals, images_t.image_sizes, targets_t, is_source=False)

        # rename rpn losses to be consistent with detection losses
        losses_t = {}
        proposal_losses_t["loss_rpn_reg"] = proposal_losses_t.pop("loss_rpn_box_reg")
        proposal_losses_t["loss_rpn_cls"] = proposal_losses_t.pop("loss_objectness")
        losses_t.update(detector_losses_t)
        losses_t.update(proposal_losses_t)
...
class SeqRoIHeads(RoIHeads):
    ...
    def forward(self, features, proposals, image_shapes, targets=None, is_source=True, query_img_as_gallery=False):
        """
        Arguments:
