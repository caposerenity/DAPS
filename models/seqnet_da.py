...
class SeqNetDa(nn.Module):
    ...
    def forward(self, images_s, targets_s=None, images_t=None, targets_t=None, query_img_as_gallery=False, is_source=False, epoch=0):
        ...
        da_ins_feas_s, da_ins_labels_s, da_ins_feas_s_before, da_ins_labels_s_before = self.roi_heads.extract_da(features_s, proposals_s, images_s.image_sizes, targets_s)
        ...
        if (epoch>=self.target_start_epoch):
            _, reid_losses_t = self.roi_heads(features_t, proposals_t, images_t.image_sizes, targets_t, query_img_as_gallery=False, is_source=False)
        ...
        if self.da_heads:
            da_losses_s = self.da_heads([features_s["feat_res4"]], da_ins_feas_s, da_ins_labels_s, da_ins_feas_s_before, da_ins_labels_s_before, targets_s)
            da_losses_t = self.da_heads([features_t["feat_res4"]], da_ins_feas_t, da_ins_labels_t, da_ins_feas_t_before, da_ins_labels_t_before, targets_t)
