...
from datasets import build_test_loader, build_train_loader_da
...
def main(args):
    ...
    print("Loading data")
    train_loader_s, train_loader_t = build_train_loader_da(cfg)
    gallery_loader, query_loader = build_test_loader(cfg)
    gallery_loader_t, query_loader_t = build_test_loader(cfg, double_test=True)
    ...
    for epoch in range(start_epoch, cfg.SOLVER.MAX_EPOCHS):
        train_one_epoch(cfg, model, optimizer, train_loader_s, train_loader_t, device, epoch, tfboard)
        lr_scheduler.step()
